...
public:
  ...
  {
    if (_tab._version != _ver)
      refresh();
    while (_ptr < _tab._t + _tab._capacity && _ptr->is_empty())
      _ptr++;
    if (_ptr == _tab._t + _tab._capacity)
    {
      _ptr = nullptr;
      _key = key_type();
    }
...
SeqCircular<E, HF, A>::begin()
{
  for (size_t i = 0; i < _capacity; ++i)
  {
    auto curr = _t[i];
    if (!curr.is_empty())
      return make_it(&_t[i], curr.getKey());
  }
  return end();
}
...
SeqCircular<E, HF, A>::cbegin() const
{
  for (size_t i = 0; i < _capacity; ++i)
  {
    auto curr = _t[i];
    if (!curr.is_empty())
      return make_cit(&_t[i], curr.getKey());
  }
  return cend();
}
